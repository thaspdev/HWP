<?php
  ...
  if (isset($_POST['usernameIn']) AND isset($_POST['passwordIn']) AND isset($_POST['passwordCheckIn']) AND isset($_POST['emailIn']) AND isset($_POST['fiNameIn']) AND isset($_POST['laNameIn']) AND isset($_POST['isWhatIn']) AND ($_POST['passwordIn'] == $_POST['passwordCheckIn'])) {
    ...
    if (isset($UConnectTest['username'])) {
      ...
      $salt = sha1(mcrypt_create_iv(10));
      $hash = sha1($salt . $_POST['passwordIn']);
      $UserRegQuery = $DB->prepare('INSERT INTO users(username, hash, salt, email, firstName, lastName, isStudent, isTeacher, isStaff) VALUES (?,?,?,?,?,?,?,?,?)');
      $UserRegQuery->execute(array(htmlspecialchars($_POST['usernameIn']), $hash, $salt, htmlspecialchars($_POST['emailIn']), htmlspecialchars($_POST['fiNameIn']), htmlspecialchars($_POST['laNameIn']), $isStu, $isTea, $isSta));
